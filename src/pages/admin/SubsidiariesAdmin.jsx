// src/pages/admin/SubsidiariesAdmin.jsx
...
export default function SubsidiariesAdmin() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("subsidiaries").select("*").order("sort_order", { ascending: true });
  ...
  const handleSubmit = async (e) => {
    ...
    let error;
    if (form.id) {
      ({ error } = await supabase.from("subsidiaries").update(payload).eq("id", form.id));
    } else {
      ({ error } = await supabase.from("subsidiaries").insert(payload));
    }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Yakin mau hapus anak perusahaan ini?")) return;
  ...
  return (
    <div className="p-6 lg:p-10">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Anak Perusahaan</h1>
          <p className="mt-1 text-sm text-slate-500">Kelola logo anak perusahaan di carousel halaman Home</p>
        </div>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-slate-100 bg-white">
        {loading ? (
          ...
          <p className="py-16 text-center text-sm text-slate-500">Belum ada data. Klik "Tambah" untuk mulai.</p>
        ) : (
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 lg:p-8">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-900">{form.id ? "Edit Anak Perusahaan" : "Tambah Anak Perusahaan"}</h2>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input name="logo_url" value={form.logo_url} onChange={handleChange} placeholder="/assets/logos/child-nama.png" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-red-400" />
                </div>
                <div>
